...
router.get('/', (req,res) => {}); // Returns current logged in user
...
router.post('/signup', async (req, res) => {
    ...
    if (existingUser) {
        return res
            .status(400)
            .json({ error: `User already with email ${email} already exists` });
    }
...
router.post('/login', async (req, res) => {
    ...
    return res.json({status: 'success', sessionId: session.id, message: 'User logged in successfully'});
